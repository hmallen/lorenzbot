...
parser.add_argument('-a', '--amount', default=1.0, type=float, help='Set static base amount of quote product to trade. [Default = 1.0 ($1)]')
...
parser.add_argument('-i', '--initial', default=0.01, type=float, help='Set proportion of total funds to use for initial buy. [Default = 0.01 (1%)]')

parser.add_argument('-m', '--max', default=10.0, type=float, help='Total amount of USDT to use for trading. [Default = 10 ($10)]')
parser.add_argument('-p', '--profit', default=0.05, type=float, help='Set profit threshold for sell triggering. [Default = 0.05 (5%)]')
parser.add_argument('-b', '--bailout', default=0.075, type=float, help='Percentage drop to trigger sell of total amount bought to start fresh. [Default = 0.075 (7.5%)]')
...
bailout_threshold = -1.0 * abs(args.bailout)
    ...
    if db[coll_current].count() == 0:
        ...
        while (True):
            ...
            try:
                ...
                logger.info('Making entry buy of ' + "{:.4f}".format(trade_amount) + ' STR at ' + "{:.4f}".format(trade_price_initial) + '.')
    ...
    if len(trade_response['resultingTrades']) > 0:
        ...
        if telegram_active == True:
            if len(connected_users) > 0:
                ...
                if position == 'buy' and telegram_delay < 900:  # If buying and last buy less than 15 min ago, don't send message
                ...
                try:
                    ...
                    logger.exception('Exception occurred while sending Telegram trade alert.')
                    logger.exception(e)
                    raise
        ...
        if withdraw_active == True and position == 'sell' and float(amount_unfilled) == 0 and skip_withdraw == False:
            ...
            try:
                withdraw_response = polo.withdraw('STR', withdraw_total, withdraw_address)
...
if __name__ == '__main__':
    ...
    while (True):
        try:
            ...
            with open(exception_log_file, 'a') as ex_file:
