...
global trade_usdt_max_start, trade_amount_start
...
if __name__ == '__main__':
    ...
    logger.debug('trade_usdt_max_start: ' + "{:.8f}".format(trade_usdt_max_start))
    trade_amount_start = trade_amount
    logger.debug('trade_amount_start: ' + "{:.8f}".format(trade_amount_start))
    ...
    if telegram_active == True:
        logger.info('Telegram logging active. Send \"/connect\" to @lorenzbot_bot to receive alerts.')  # BOT NAME WILL CHANGE FOR OTHER USERS
        time.sleep(5)

    logger.info('Configuration complete. Beginning program...')
    ...
    while (True):
        try:
            ...
            if trade_check_ready == True:
                ...
                # Display INFO level logging data for user
                logger.info('Base Price: ' + "{:.6f}".format(base_price))
                logger.info('Base Target: ' + "{:.6f}".format(base_price_target))
                logger.info('Sell Target: ' + "{:.6f}".format(sell_price_target))
                logger.info('Current Buy Amount: ' + "{:.4f}".format(buy_amount_current))
            ...
            logger.info('Sleep time: ' + "{:.2f}".format(loop_time_dynamic) + ' sec.')
            loop_count += 1
            ...
            logger.info('Copied logs to root directory.')
